...
void StaticRecompCore::Init()
{
  ...
  const char* native_burst = std::getenv("STATICRECOMP_NATIVE_BURST");
  m_native_burst_enabled = native_burst &&
      (std::strcmp(native_burst, "1") == 0 ||
       std::strcmp(native_burst, "true") == 0 ||
       std::strcmp(native_burst, "yes") == 0 ||
