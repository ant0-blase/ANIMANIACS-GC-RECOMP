...
int StaticRecompCore::ChunkIndexOf(u32 address)
{
  ...
  if (m_active_rel_sections.empty())
  {
    ...
    /*
     * PERF: last-chunk locality fast path.
     *
     * Native control flow very often stays inside the same 0x4000-byte
     * recomp chunk.  Avoid recomputing the candidate and touching another
     * range entry when the current PC is still in the previous chunk.
     *
     * The actual module range is always validated, so this remains safe
     * for irregular/tail chunks.
     */
    const u32 last = m_last_chunk_index;

    if (last < num_chunks)
    {
      const auto& chunk = ranges[last];

      if (address >= chunk.start &&
          address < chunk.end &&
          m_chunk_rel_sections[last] < 0)
      {
  ...
  const int result = m_chunk_lookup_table[idx];

  if (result >= 0)
    m_last_chunk_index = static_cast<u32>(result);
